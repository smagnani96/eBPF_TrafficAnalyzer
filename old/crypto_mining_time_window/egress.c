...
BPF_QUEUESTACK("extern", PACKET_BUFFER_CRYPTO, struct features, N_PACKET_TOTAL, 0);

/*Tracked session LRU map, value = server address*/
BPF_TABLE("extern", struct session_key, __be32, SESSIONS_TRACKED_CRYPTO, N_SESSION);
...
static __always_inline int handle_rx(struct CTXTYPE *ctx, struct pkt_metadata *md) {
  ...
  switch (ip->protocol) {
    case IPPROTO_TCP: {
      ...
      update_session(ctx, &key, ip->saddr);
