...
#define N_SESSION 10
#define N_PACKET_TOTAL 1000                         //Number of max packet captured (Size of PACKET_BUFFER)
...
struct features {
    struct session_key key;                         // The message identifier
    uint64_t n_bits;                                // Number of bit in this message
...
BPF_QUEUESTACK_SHARED("queue", PACKET_BUFFER_CRYPTO, struct features, N_PACKET_TOTAL, 0);

/*Tracked session LRU map, value = server address*/
BPF_TABLE_SHARED("lru_hash", struct session_key, __be32, SESSIONS_TRACKED_CRYPTO, N_SESSION);

static __always_inline void update_session(struct CTXTYPE *ctx, struct session_key *key, __be32 server_addr) {
  __be32 *value = SESSIONS_TRACKED_CRYPTO.lookup(key);
  if (!value) {
    /*New session accepted*/
    /*TODO : SERVER = srcAddress by now*/
    SESSIONS_TRACKED_CRYPTO.insert(key, &server_addr);
  }
}
...
static __always_inline int handle_rx(struct CTXTYPE *ctx, struct pkt_metadata *md) {
  ...
  switch (ip->protocol) {
    case IPPROTO_TCP: {
      ...
      update_session(ctx, &key, ip->daddr);
      ...
      struct features new_val = {.key=key, .n_bits=bpf_ntohs(ip->tot_len), .timestamp=curr_time};
      PACKET_BUFFER_CRYPTO.push(&new_val, BPF_EXIST);
